...
def calculate_data_rate(freqs, formats, details=False):
...
tests = [
    ("Log records", LOG_RATES_HZ, LOG_RECORD_FORMATS, True),
    ("Telemetry", TELEM_RATES_HZ, {**LOG_RECORD_FORMATS, **TELEM_RECORD_FORMATS}, False)
]


for title, rates, records, estimate_duration in tests:
    print(f"# {title}")
    print()
